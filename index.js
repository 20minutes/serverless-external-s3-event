...
class S3Deploy {
  ...
  checkBucketsExist() {
    this.consoleLog(`External s3 event: ${this.yellow('Checking existing buckets actually exist')}`);
    ...
    return this.provider
      .request('S3', 'listBuckets', {}, `${this.serverless.service.provider.stage}`, `${this.serverless.service.provider.region}`)
      .then((returnedBuckets)=>{
        ...
        return this.consoleLog(`External s3 event: ${this.yellow('All existing buckets actually exist')}`);
      });
  }
  ...
  getBucketNotifications() {
    ...
    let lambdaConfigs = funcObjs
      .map(obj => this.getLambdaFunctionConfigurationsFromFunction(obj))
    ...
    return this.getFunctionArnFromDeployedStack(info, deployed.deployedName).then((arn) => {
      ...
      if ("Filter" in cfg) {
        ...
        if (cfg['Filter']['Key']['FilterRules'][0]['Name'] == 'prefix') {
          filterText.push(this.yellow("prefix: " + cfg['Filter']['Key']['FilterRules'][0]['Value']))
        }
        if (cfg['Filter']['Key']['FilterRules'][0]['Name'] == 'suffix') {
          filterText.push(this.yellow("suffix: " + cfg['Filter']['Key']['FilterRules'][0]['Value']))
        }
        if (cfg['Filter']['Key']['FilterRules'].length > 1) {
          if (cfg['Filter']['Key']['FilterRules'][1]['Name'] == 'prefix') {
            filterText.push(this.yellow("prefix: " + cfg['Filter']['Key']['FilterRules'][1]['Value']))
          }
          if (cfg['Filter']['Key']['FilterRules'][1]['Name'] == 'suffix') {
            filterText.push(this.yellow("suffix: " + cfg['Filter']['Key']['FilterRules'][1]['Value']))
          }
        }
      }
      ...
      this.consoleLog(`Attaching ${this.yellow(deployed.deployedName)} to ${this.yellow(bucket.Bucket)} on ${this.yellow(cfg.Events)} with ${filterText.join(', ')}`);
  ...
  afterS3DeployFunctions() {
    this.serverless.cli.consoleLog(this.yellow(this.underline('External s3 event')))
  ...
  s3EventApi(cfg) {
    ...
    return this.provider
      .request('S3', 'getBucketNotificationConfiguration', { Bucket: cfg.Bucket }, `${this.serverless.service.provider.stage}`, `${this.serverless.service.provider.region}`)
      .then((bucketConfig) => {
        ...
        for (var i = 0; i < cfg['NotificationConfiguration']['LambdaFunctionConfigurations'].length; i++) {
          ...
          if (!found) {
            bucketConfig['LambdaFunctionConfigurations'].push(cfg['NotificationConfiguration']['LambdaFunctionConfigurations'][i])
          }
        }
  ...
  lambdaPermApi(cfg) {
    ...
    if (this.functionPolicies[cfg.FunctionName]) {
      ...
      existingPolicyPromise = this.provider
        .request('Lambda', 'getPolicy', { FunctionName: cfg.FunctionName }, `${this.serverless.service.provider.stage}`, `${this.serverless.service.provider.region}`)
        .then((result) => {
  ...
  s3BucketRemoveEvent () {
    ...
    return Promise.all(bucketNotifications.map((cfg) => {
      return this.provider
        ...
        .then((bucketConfig) => {
          ...
          cfg.NotificationConfiguration.LambdaFunctionConfigurations.forEach((ourcfg) => {
            this.consoleLog(`Removing ${ourcfg.LambdaFunctionArn} from ${cfg.Bucket} ${ourcfg.Events}...`);
      ...
      .then(() => this.consoleLog('Removed all existing bucket events'));
  }
